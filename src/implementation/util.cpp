  ...
  unsigned int lineOffset, bool bHlsl = false, bool withDebugInfo = false)
{
	...
	if(shaderPtr->get_spirv_blob_size() == 0u) {
		...
		if(shaderInfoLog.empty() == false) {
			if(infoLog != nullptr) {
				prosper::glsl::translate_error(shaderCode, shaderInfoLog, fileName, includeLines, static_cast<uint32_t>(lineOffset), infoLog);
			...
			if(programInfoLog.empty() == false) {
				if(infoLog != nullptr) {
					prosper::glsl::translate_error(shaderCode, programInfoLog, fileName, includeLines, static_cast<uint32_t>(lineOffset), infoLog);
					*infoLog = std::string("Shader File: \"") + fileName + "\"\n" + (*infoLog);
				}
				if(debugInfoLog != nullptr)
