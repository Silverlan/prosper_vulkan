...
std::shared_ptr<VlkImage> VlkImage::Create(IPrContext &context, std::unique_ptr<Anvil::Image, std::function<void(Anvil::Image *)>> img, const prosper::util::ImageCreateInfo &createInfo, bool isSwapchainImage, const std::function<void(VlkImage &)> &onDestroyedCallback)
{
	if(img == nullptr)
		return nullptr;
...
VlkImage::VlkImage(IPrContext &context, std::unique_ptr<Anvil::Image, std::function<void(Anvil::Image *)>> img, const prosper::util::ImageCreateInfo &createInfo, bool isSwapchainImage) : IImage {context, createInfo}, m_image {std::move(img)}, m_swapchainImage {isSwapchainImage}
{
	if(m_swapchainImage)
		return;
...
void VlkImage::Bake() { GetAnvilImage().get_image(); }
...
bool VlkImage::Map(DeviceSize offset, DeviceSize size, void **outPtr) { return m_image->get_memory_block()->map(offset, size, outPtr); }
...
bool VlkImage::DoSetMemoryBuffer(prosper::IBuffer &buffer) { return m_image->set_memory(buffer.GetAPITypeRef<VlkBuffer>().GetAnvilBuffer().get_memory_block(0)); }
