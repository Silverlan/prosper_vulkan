...
VlkShaderStageProgram::VlkShaderStageProgram(std::vector<unsigned int> &&spirvBlob) : m_spirvBlob {std::move(spirvBlob)} {}
...
bool VkRaytracingFunctions::IsValid() const { return vkCreateAccelerationStructureKHR && vkDestroyAccelerationStructureKHR; }
...
std::shared_ptr<VlkContext> VlkContext::Create(const std::string &appName, bool bEnableValidation) { return std::shared_ptr<VlkContext> {new VlkContext {appName, bEnableValidation}}; }
...
{
	return m_devicePtr->get_physical_device_image_format_properties(Anvil::ImageFormatPropertiesQuery {static_cast<Anvil::Format>(format), static_cast<Anvil::ImageType>(type), static_cast<Anvil::ImageTiling>(tiling), static_cast<Anvil::ImageUsageFlagBits>(usageFlags), {}});
}
...
prosper::FeatureSupport VlkContext::AreFormatFeaturesSupported(Format format, FormatFeatureFlags featureFlags, std::optional<ImageTiling> tiling) const
{
	...
	constexpr std::array<Anvil::FormatFeatureFlagBits, 8> toAnvFlag {Anvil::FormatFeatureFlagBits::BLIT_DST_BIT, Anvil::FormatFeatureFlagBits::BLIT_SRC_BIT, Anvil::FormatFeatureFlagBits::COLOR_ATTACHMENT_BIT, Anvil::FormatFeatureFlagBits::DEPTH_STENCIL_ATTACHMENT_BIT,
...
std::optional<prosper::util::PhysicalDeviceImageFormatProperties> VlkContext::GetPhysicalDeviceImageFormatProperties(const ImageFormatPropertiesQuery &query)
{
	...
	if(dev.get_physical_device_image_format_properties(Anvil::ImageFormatPropertiesQuery {static_cast<Anvil::Format>(query.format), static_cast<Anvil::ImageType>(query.imageType), static_cast<Anvil::ImageTiling>(query.tiling), static_cast<Anvil::ImageUsageFlagBits>(query.usageFlags), {}},
	     &imgFormatProperties)
	  == false)
		return {};
...
std::shared_ptr<prosper::ISecondaryCommandBuffer> VlkContext::AllocateSecondaryLevelCommandBuffer(prosper::QueueFamilyType queueFamilyType, uint32_t &universalQueueFamilyIndex)
{
	...
	return std::dynamic_pointer_cast<ISecondaryCommandBuffer>(prosper::VlkSecondaryCommandBuffer::Create(*this, GetDevice().get_command_pool_for_queue_family_index(universalQueueFamilyIndex)->alloc_secondary_level_command_buffer(), queueFamilyType));
}
...
std::unique_ptr<prosper::ShaderModule> VlkContext::CreateShaderModuleFromStageData(const std::shared_ptr<ShaderStageProgram> &shaderStageProgram, prosper::ShaderStage stage, const std::string &entryPointName)
{
	return std::make_unique<prosper::ShaderModule>(shaderStageProgram, (stage == prosper::ShaderStage::Compute) ? entryPointName : "", (stage == prosper::ShaderStage::Fragment) ? entryPointName : "", (stage == prosper::ShaderStage::Geometry) ? entryPointName : "",
	  (stage == prosper::ShaderStage::TessellationControl) ? entryPointName : "", (stage == prosper::ShaderStage::TessellationEvaluation) ? entryPointName : "", (stage == prosper::ShaderStage::Vertex) ? entryPointName : "");
}
...
std::shared_ptr<prosper::IBuffer> VlkContext::CreateBuffer(const prosper::util::BufferCreateInfo &createInfo, const void *data)
{
	...
	if((createInfo.flags & prosper::util::BufferCreateInfo::Flags::DontAllocateMemory) == prosper::util::BufferCreateInfo::Flags::None) {
		...
		if(m_useAllocator) {
			auto bufferCreateInfo = Anvil::BufferCreateInfo::create_no_alloc(&dev, static_cast<VkDeviceSize>(createInfo.size), queue_family_flags_to_anvil_queue_family(createInfo.queueFamilyMask), sharingMode, createFlags, static_cast<Anvil::BufferUsageFlagBits>(createInfo.usageFlags));
		...
		for(auto iLayer = decltype(createInfo.layers) {0u}; iLayer < createInfo.layers; ++iLayer) {
			for(auto iMipmap = decltype(numMipmaps) {0u}; iMipmap < numMipmaps; ++iMipmap) {
				...
				if(mipmapData == nullptr)
					continue;
				anvMipmapData.emplace_back(Anvil::MipmapRawData::create_2D_from_uchar_ptr(util::is_depth_format(createInfo.format) ? Anvil::ImageAspectFlagBits::DEPTH_BIT : Anvil::ImageAspectFlagBits::COLOR_BIT, iMipmap, mipmapData, dataSize, rowSize));
			}
		}
	}
...
void *VlkContext::GetInternalDevice() const { return GetDevice().get_device_vk(); }
void *VlkContext::GetInternalPhysicalDevice() const { return GetDevice().get_physical_device()->get_physical_device(); }
void *VlkContext::GetInternalInstance() const { return GetDevice().get_parent_instance()->get_instance_vk(); }
void *VlkContext::GetInternalUniversalQueue() const { return GetDevice().get_universal_queue(0u)->get_queue(); }
bool VlkContext::IsDeviceExtensionEnabled(const std::string &ext) const { return GetDevice().is_extension_enabled(ext); }
bool VlkContext::IsInstanceExtensionEnabled(const std::string &ext) const { return GetAnvilInstance().is_instance_extension_enabled(ext); }
...
bool VlkContext::GetUniversalQueueFamilyIndex(prosper::QueueFamilyType queueFamilyType, uint32_t &queueFamilyIndex) const
{
	...
	if(m_devicePtr->get_queue_family_indices_for_queue_family_type(static_cast<Anvil::QueueFamilyType>(queueFamilyType), &n_universal_queue_family_indices, &universal_queue_family_indices) == false || n_universal_queue_family_indices == 0u)
		return false;
...
void VlkContext::InitMainRenderPass()
{
	...
	render_pass_info_ptr->add_color_attachment(swapchain.get_create_info_ptr()->get_format(), Anvil::SampleCountFlagBits::_1_BIT, Anvil::AttachmentLoadOp::CLEAR, Anvil::AttachmentStoreOp::STORE, Anvil::ImageLayout::COLOR_ATTACHMENT_OPTIMAL, Anvil::ImageLayout::COLOR_ATTACHMENT_OPTIMAL,
	  false, /* may_alias */
	  &render_pass_color_attachment_id);
	render_pass_info_ptr->add_subpass(&m_mainSubPass);
	render_pass_info_ptr->add_subpass_color_attachment(m_mainSubPass, Anvil::ImageLayout::COLOR_ATTACHMENT_OPTIMAL, render_pass_color_attachment_id, 0, /* in_location */
			...
			if(computePipeline == false || stage == prosper::ShaderStage::Compute) {
				for(auto j = decltype(specializationConstants->size()) {0u}; j < specializationConstants->size(); ++j) {
					...
					if(computePipeline == false) {
						static_cast<Anvil::GraphicsPipelineCreateInfo &>(anvPipelineCreateInfo).add_specialization_constant(static_cast<Anvil::ShaderStage>(stage), specializationConstant.constantId, specializationConstant.numBytes, dataBuffer + specializationConstant.startOffset);
						continue;
					}
					static_cast<Anvil::ComputePipelineCreateInfo &>(anvPipelineCreateInfo).add_specialization_constant(specializationConstant.constantId, specializationConstant.numBytes, dataBuffer + specializationConstant.startOffset);
				}
			}
		}
	}
...
bool prosper::VlkContext::GetParsedShaderSourceCode(prosper::Shader &shader, std::vector<std::string> &outGlslCodePerStage, std::vector<prosper::ShaderStage> &outGlslCodeStages, std::string &outInfoLog, std::string &outDebugInfoLog, prosper::ShaderStage &outErrStage) const
{
...
std::optional<prosper::PipelineID> VlkContext::AddPipeline(prosper::Shader &shader, PipelineID shaderPipelineId, const prosper::ComputePipelineCreateInfo &createInfo, prosper::ShaderStageData &stage, PipelineID basePipelineId)
{
	PipelineID pipelineId;
...
std::optional<PipelineID> VlkContext::AddPipeline(prosper::Shader &shader, PipelineID shaderPipelineId, const prosper::RayTracingPipelineCreateInfo &createInfo, prosper::ShaderStageData &stage, PipelineID basePipelineId)
{
	// TODO: This is a stub
	return {};
}

std::optional<prosper::PipelineID> prosper::VlkContext::AddPipeline(prosper::Shader &shader, PipelineID shaderPipelineId, const prosper::GraphicsPipelineCreateInfo &createInfo, IRenderPass &rp, prosper::ShaderStageData *shaderStageFs, prosper::ShaderStageData *shaderStageVs,
  prosper::ShaderStageData *shaderStageGs, prosper::ShaderStageData *shaderStageTc, prosper::ShaderStageData *shaderStageTe, SubPassID subPassId, PipelineID basePipelineId)
{
	...
	for(auto attId = decltype(numBlendAttachments) {0u}; attId < numBlendAttachments; ++attId) {
		...
		if(createInfo.GetColorBlendAttachmentProperties(attId, &blendingEnabled, &blendOpColor, &blendOpAlpha, &srcColorBlendFactor, &dstColorBlendFactor, &srcAlphaBlendFactor, &dstAlphaBlendFactor, &channelWriteMask)) {
			gfxPipelineInfo->set_color_blend_attachment_properties(attId, blendingEnabled, static_cast<Anvil::BlendOp>(blendOpColor), static_cast<Anvil::BlendOp>(blendOpAlpha), static_cast<Anvil::BlendFactor>(srcColorBlendFactor), static_cast<Anvil::BlendFactor>(dstColorBlendFactor),
			  static_cast<Anvil::BlendFactor>(srcAlphaBlendFactor), static_cast<Anvil::BlendFactor>(dstAlphaBlendFactor), static_cast<Anvil::ColorComponentFlagBits>(channelWriteMask));
		}
	}
	...
	createInfo.GetDepthBiasState(&isDepthBiasStateEnabled, &depthBiasConstantFactor, &depthBiasClamp, &depthBiasSlopeFactor);
	...
	createInfo.GetGraphicsPipelineProperties(&numScissors, &numViewports, &numVertexBindings, &renderPass, &subPassId);
	...
	LogicOp logicOp;
	createInfo.GetLogicOpState(&isLogicOpEnabled, &logicOp);
	...
	gfxPipelineInfo->set_rasterization_properties(static_cast<Anvil::PolygonMode>(polygonMode), static_cast<Anvil::CullModeFlagBits>(cullMode), static_cast<Anvil::FrontFace>(frontFace), lineWidth);
	...
	createInfo.GetStencilTestProperties(&isStencilTestEnabled, &frontStencilFailOp, &frontStencilPassOp, &frontStencilDepthFailOp, &frontStencilCompareOp, &frontStencilCompareMask, &frontStencilWriteMask, &frontStencilReference, &backStencilFailOp, &backStencilPassOp,
	  &backStencilDepthFailOp, &backStencilCompareOp, &backStencilCompareMask, &backStencilWriteMask, &backStencilReference);
	gfxPipelineInfo->toggle_stencil_test(isStencilTestEnabled);
	gfxPipelineInfo->set_stencil_test_properties(true, static_cast<Anvil::StencilOp>(frontStencilFailOp), static_cast<Anvil::StencilOp>(frontStencilPassOp), static_cast<Anvil::StencilOp>(frontStencilDepthFailOp), static_cast<Anvil::CompareOp>(frontStencilCompareOp),
	  frontStencilCompareMask, frontStencilWriteMask, frontStencilReference);
	gfxPipelineInfo->set_stencil_test_properties(false, static_cast<Anvil::StencilOp>(backStencilFailOp), static_cast<Anvil::StencilOp>(backStencilPassOp), static_cast<Anvil::StencilOp>(backStencilDepthFailOp), static_cast<Anvil::CompareOp>(backStencilCompareOp), backStencilCompareMask,
...
std::shared_ptr<prosper::IImage> create_image(prosper::IPrContext &context, const prosper::util::ImageCreateInfo &pImgCreateInfo, const std::vector<std::shared_ptr<uimg::ImageBuffer>> &imgBuffers, bool cubemap)
{
	...
	for(auto iLayer = decltype(imgBuffers.size()) {0u}; iLayer < imgBuffers.size(); ++iLayer) {
		...
		if(cubemap) {
			layers.push_back(Anvil::MipmapRawData::create_cube_map_from_uchar_ptr(Anvil::ImageAspectFlagBits::COLOR_BIT, iLayer, 0, static_cast<uint8_t *>(imgBuf->GetData()), imgCreateInfo.width * imgCreateInfo.height * byteSize, imgCreateInfo.width * byteSize));
		}
		else {
			layers.push_back(Anvil::MipmapRawData::create_2D_from_uchar_ptr(Anvil::ImageAspectFlagBits::COLOR_BIT, 0u, static_cast<uint8_t *>(imgBuf->GetData()), imgCreateInfo.width * imgCreateInfo.height * byteSize, imgCreateInfo.width * byteSize));
		}
	}
...
std::shared_ptr<prosper::IImage> prosper::VlkContext::CreateImage(const util::ImageCreateInfo &createInfo, const std::vector<Anvil::MipmapRawData> &data) { return ::create_image(*this, createInfo, &data); }
	...
	for(auto i = decltype(numSets) {0}; i < numSets; ++i) {
		...
		for(auto j = decltype(numBindings) {0}; j < numBindings; ++j) {
			...
			case Anvil::DescriptorType::UNIFORM_BUFFER:
				{
					std::vector<Anvil::DescriptorSet::UniformBufferBindingElement> bindingElements(arraySize, Anvil::DescriptorSet::UniformBufferBindingElement {&dummyBuf->GetAPITypeRef<VlkBuffer>().GetAnvilBuffer(), 0ull, dummyBuf->GetSize()});
					descSet->set_binding_array_items(bindingIndex, {0u, arraySize}, bindingElements.data());
					break;
				}
			case Anvil::DescriptorType::UNIFORM_BUFFER_DYNAMIC:
				{
					std::vector<Anvil::DescriptorSet::DynamicUniformBufferBindingElement> bindingElements(arraySize, Anvil::DescriptorSet::DynamicUniformBufferBindingElement {&dummyBuf->GetAPITypeRef<VlkBuffer>().GetAnvilBuffer(), 0ull, dummyBuf->GetSize()});
					descSet->set_binding_array_items(bindingIndex, {0u, arraySize}, bindingElements.data());
					break;
				}
			case Anvil::DescriptorType::STORAGE_BUFFER:
				{
					std::vector<Anvil::DescriptorSet::StorageBufferBindingElement> bindingElements(arraySize, Anvil::DescriptorSet::StorageBufferBindingElement {&dummyBuf->GetAPITypeRef<VlkBuffer>().GetAnvilBuffer(), 0ull, dummyBuf->GetSize()});
					descSet->set_binding_array_items(bindingIndex, {0u, arraySize}, bindingElements.data());
					break;
				}
			case Anvil::DescriptorType::STORAGE_BUFFER_DYNAMIC:
				{
					std::vector<Anvil::DescriptorSet::DynamicStorageBufferBindingElement> bindingElements(arraySize, Anvil::DescriptorSet::DynamicStorageBufferBindingElement {&dummyBuf->GetAPITypeRef<VlkBuffer>().GetAnvilBuffer(), 0ull, dummyBuf->GetSize()});
					descSet->set_binding_array_items(bindingIndex, {0u, arraySize}, bindingElements.data());
					break;
				}
			}
...
bool VlkContext::QueryResult(const PipelineStatisticsQuery &query, PipelineStatistics &outStatistics) const { return QueryResult<PipelineStatistics, uint64_t>(query, outStatistics, prosper::QueryResultFlags::e64Bit); }
std::shared_ptr<prosper::ISampler> prosper::VlkContext::CreateSampler(const util::SamplerCreateInfo &createInfo) { return VlkSampler::Create(*this, createInfo); }

std::shared_ptr<prosper::IEvent> prosper::VlkContext::CreateEvent() { return VlkEvent::Create(*this); }
std::shared_ptr<prosper::IFence> prosper::VlkContext::CreateFence(bool createSignalled) { return VlkFence::Create(*this, createSignalled, nullptr); }
std::shared_ptr<prosper::IImageView> prosper::VlkContext::DoCreateImageView(const prosper::util::ImageViewCreateInfo &createInfo, prosper::IImage &img, Format format, ImageViewType type, prosper::ImageAspectFlags aspectMask, uint32_t numLayers)
{
	switch(type) {
...
std::shared_ptr<prosper::IRenderPass> prosper::VlkContext::CreateRenderPass(const util::RenderPassCreateInfo &renderPassInfo)
{
	...
	for(auto &attInfo : renderPassInfo.attachments) {
		...
		if(util::is_depth_format(static_cast<prosper::Format>(attInfo.format)) == false) {
			rpInfo->add_color_attachment(static_cast<Anvil::Format>(attInfo.format), static_cast<Anvil::SampleCountFlagBits>(attInfo.sampleCount), static_cast<Anvil::AttachmentLoadOp>(attInfo.loadOp), static_cast<Anvil::AttachmentStoreOp>(attInfo.storeOp),
			  static_cast<Anvil::ImageLayout>(attInfo.initialLayout), static_cast<Anvil::ImageLayout>(attInfo.finalLayout), true, &attId);
		}
		else {
			...
			rpInfo->add_depth_stencil_attachment(static_cast<Anvil::Format>(attInfo.format), static_cast<Anvil::SampleCountFlagBits>(attInfo.sampleCount), static_cast<Anvil::AttachmentLoadOp>(attInfo.loadOp), static_cast<Anvil::AttachmentStoreOp>(attInfo.storeOp),
			  static_cast<Anvil::AttachmentLoadOp>(attInfo.stencilLoadOp), static_cast<Anvil::AttachmentStoreOp>(attInfo.stencilStoreOp), static_cast<Anvil::ImageLayout>(attInfo.initialLayout), static_cast<Anvil::ImageLayout>(attInfo.finalLayout), true, &attId);
	...
	if(renderPassInfo.subPasses.empty() == false) {
		for(auto &subPassInfo : renderPassInfo.subPasses) {
			...
			for(auto &dependency : subPassInfo.dependencies) {
				rpInfo->add_subpass_to_subpass_dependency(dependency.destinationSubPassId, dependency.sourceSubPassId, static_cast<Anvil::PipelineStageFlagBits>(dependency.destinationStageMask), static_cast<Anvil::PipelineStageFlagBits>(dependency.sourceStageMask),
				  static_cast<Anvil::AccessFlagBits>(dependency.destinationAccessMask), static_cast<Anvil::AccessFlagBits>(dependency.sourceAccessMask), Anvil::DependencyFlagBits::NONE);
			}
		}
	}
	else {
...
{
	auto createInfo = Anvil::FramebufferCreateInfo::create(&static_cast<VlkContext &>(*this).GetDevice(), width, height, layers);
	uint32_t depth = 1u;
	for(auto *att : attachments)
		createInfo->add_attachment(&static_cast<prosper::VlkImageView *>(att)->GetAnvilImageView(), nullptr);
	return prosper::VlkFramebuffer::Create(*this, attachments, width, height, depth, layers, Anvil::Framebuffer::create(std::move(createInfo)));
}
...
std::shared_ptr<prosper::IRenderBuffer> prosper::VlkContext::CreateRenderBuffer(const prosper::GraphicsPipelineCreateInfo &pipelineCreateInfo, const std::vector<prosper::IBuffer *> &buffers, const std::vector<prosper::DeviceSize> &offsets,
